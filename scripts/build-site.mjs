...
const sources=[...scan('docs'),...scan('templates'),'knowledge.md','sources.md',...scan('.agents/skills/ai-management-decision-coach')];
...
const groups=[['任务与价值',sources.filter(s=>/docs\/cards\/0[123]-/.test(s))],['知识与共同语言',sources.filter(s=>/docs\/cards\/0[456]-/.test(s))],['智能体与授权',sources.filter(s=>/docs\/cards\/0[789]-/.test(s))],['评测与投资',sources.filter(s=>/docs\/cards\/(10|11|12)-/.test(s))],['数据与学习',sources.filter(s=>/docs\/cards\/(13|14|15)-/.test(s))]];
...
<section class="section shade" id="library"><div class="wrap"><div class="section-head"><h2>十五张管理知识卡</h2><p>按问题查找，全部在本站阅读。</p></div><div class="library-grid">${groups.map(([g,ss])=>`<section class="group"><h3>${g}</h3>${ss.map(s=>row(s)).join('')}</section>`).join('')}</div></div></section>
